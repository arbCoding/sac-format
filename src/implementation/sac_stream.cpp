...
void SacStream::detrend()
{
  if (iftype != 1)
  {
    std::cerr << "Incorrect `iftype`: expected 1 (SAC ITIME), receiver: `" << iftype << "`\n";
    return;
  }
  const double y_mean{npts / 2.0};
  const double x_mean{mean()};
  double numerator{0.0};
  double denominator{0.0};
  for (std::size_t i{0}; i < data1.size(); ++i)
  {
    numerator += (data1[i] - x_mean) * (i - y_mean);
    denominator += (data1[i] - x_mean) * (data1[i] - x_mean);
  }
  const double slope{numerator / denominator};
  const double y_intercept{y_mean - (slope * x_mean)};
  // Now to remove the trend
  for (std::size_t i{0}; i < data1.size(); ++i)
  {
    data1[i] -= (slope * i) + y_intercept;
  }
}
//-----------------------------------------------------------------------------
// End time-series preprocessing functions
//-----------------------------------------------------------------------------
