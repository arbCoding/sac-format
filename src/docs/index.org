...
anywhere in the world. sac-format's operating system agnostic and confirmed to
run on Windows, macOS, and Linux systems.
...
are tested and confirmed working. New tests added when they're imagined.
*** Fast
sac-format's *fast*---it's written in C++ and extensively benchmarked. You can run
the benchmarks yourself to find how sac-format performs on your system.
*** Easy
sac-format's *easy*---single-header makes integration to any project a breeze.
[[https://cmake.org/][CMake]] makes building on different platforms a breeze. Object-oriented design
makes use simple and intuitive. See the [[Quickstart]] section to get up and
running.
*** Compact
sac-format's *compact*---in total (header + implementation--excluding comments)
there are fewer than 1500 lines of code. This small size makes it easy to use on
any sort of hardware (such as a [[https://www.raspberrypi.com/][Raspberry Pi]]).
*** Documentation
...
sac-format aims to be accessible, modern, safe, and fast---C++20 provides the
best fit.
** Dependencies
