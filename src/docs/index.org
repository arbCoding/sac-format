...
Download [[https://github.com/arbCoding/sac-format/releases][sac-format]] from the GitHub release page.

[[https://arbcoding.github.io/sac-format/sac-format_manual.pdf][Download]] an offline version of the documentation (PDF).

Get [[https://github.com/arbCoding/sac-format/discussions/][help]] from the community forum.
...
of conformance checking are [[https://github.com/arbCoding/sac-format/actions/workflows/cpp-linter.yml][here]].
...
[[https://cmake.org/][CMake]], even on different platforms. Object-oriented design makes use easy and
intuitive. See the [[Quickstart][Quickstart]] section to get up and running.
...
- [[https://app.codecov.io/gh/arbCoding/sac-format][CodeCov]]
- [[https://scan.coverity.com/projects/arbcoding-sac-format][Coverity Scan]]
...
Installers for the latest release are located [[https://github.com/arbCoding/sac-format/releases/latest][here]]. Be sure to check the sha512
...
sac-format provides a graphical installer on Windows (=sac-format.exe=)\ast{}.

\ast{} You will get a warning from Windows Defender that this is from an unknown
developer. To run the installer you'll need to click on 'More Info' and then
agree to run anyway. If you're unsure of the provenance of the file, be sure to
only download from the [[https://github.com/arbCoding/sac-format/releases][releases]] page and to check the sha512 checksum of the
installer ([[https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.utility/get-filehash?view=powershell-7.4][more info here]]) against =sac-format.exe.sha512=.
...
bash sac-format-<version>-Darwin-<arch>.tar.gz
#+end_src
...
#+attr_latex: :options frame=lines, framesep=2mm, baselinestretch=1.2, fontsize=\footnotesize, bgcolor=LightGray
#+begin_src bash :returns code :eval no
# Ceck the sha512 checksum
sha512sum -c sac-format-<version>-Linux-<arch>tar.gz.sha512
bash sac-format-<version>-Linux-<arch>.tar.gz
#+end_src
...
[[https://en.cppreference.com/w/cpp/error/exception][=std::exception=]]) in the event of a failure to read/write a SAC-file.
