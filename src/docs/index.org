...
#+latex_header: \fancyhead[C]{sac-format} \fancyhead[LO, RE]{\emph{User Manual}} \fancyhead[LE, RO]{\today}
...
*** Automatic (CMake)
1) [[https://github.com/Reputeless/Xoshiro-cpp][Xoshiro-cpp v1.12.0]] (=utests.cpp= only)
2) [[https://github.com/catchorg/Catch2][Catch2 v3.4.0]] (=utests.cpp= only)
** Trace class
*** Structure
**** 'floats' array
**** 'doubles' array
...
** Benchmarking
** SAC-file format
The official and up-to-date documentation is available from the EarthScope
Consortium (formerly IRIS/UNAVCO) [[https://ds.iris.edu/files/sac-manual/manual/file_format.html][here]]. The following subsections
constitute my notes on the format. They are meant to act as a useful guide---all
...
|--------+----------+------------------+-------------------------+------------------------|
| -12345 | UNDEF    | (/Historic/) Earth |               6378160.0 |             0.00335293 |
|     98 | ISUN     | Sun              |             696000000.0 |               8.189e-6 |
|     99 | IMERCURY | Mercury          |               2439700.0 |                    0.0 |
|    100 | IVENUS   | Venus            |               6051800.0 |                    0.0 |
|    101 | IEARTH   | (/WGS84/) Earth    |               6378137.0 |  0.0033528106647474805 |
...
If true, then calculate [[=dist=][=dist=]], [[=az=][=az=]], [[=baz=][=baz=]], and [[=gcarc=][=gcarc=]] from [[=stla=][=stla=]], [[=stlo=][=stlo=]], [[=evla=][=evla=]], and [[=evlo=][=evlo=]].
*** String (23)
